...
class DummyDataset(InMemoryComplexDataset):
    ...
    def factory():
        ...
        for c, complex in enumerate(complexes):
            complex.y = torch.LongTensor([c % 2]) # dummy labels for complexes
        return complexes
...
class DummyMolecularDataset(InMemoryComplexDataset):
    ...
    def factory(remove_2feats=False):
        ...
        for c, complex in enumerate(complexes):
            if remove_2feats:
                if 2 in complex.cochains: # remove the edge features. (bonds Molecular datasets shouldn't carry features)
