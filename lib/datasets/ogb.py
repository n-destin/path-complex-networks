...
class OGBDataset(InMemoryComplexDataset):
    ...
    def __init__(self, root, name, use_edge_features=False, transform=None, max_dim = 2,
                 ...
                 simple=False, include_down_adj=False, n_jobs=2, type = "graph", **kwargs):
        self.name = name
        self.type = type
    ...
    def download(self):
        ...
        dataset = get_ogb_dataset(self.type)(self.name, self.raw_dir)
    ...
    def process(self): # called when accessing processed_data[index] due to __getitem__ and __len__ attributes of Dataset
        
        # At this stage, the graph dataset is already downloaded and processed
        dataset = get_ogb_dataset(self.type)(self.name, self.raw_dir) # load the ogb dataset
        split_idx = dataset.get_idx_split() # built-in train_validation splits
        if self._simple:  # Using only two-dimensional features
...
def load_ogb_graph_dataset(root, type, name):
    raw_dir = osp.join(root, 'raw')
    dataset = get_ogb_dataset(type)(name, root)
    if name.startswith('ogbl'):
        idx = dataset.get_edge_split()
    else:
        idx = dataset.get_idx_split()
    num_classes = dataset.num_features
    num_features = dataset[0].x.shape[1]

    return dataset, idx['train'], idx['valid'], idx['test'], num_classes, num_features


def get_ogb_dataset(type):
    dataset = None
    if type == "graph":
    ...
    else: raise ValueError("Invalid ogb datasset type {}".format(type))
