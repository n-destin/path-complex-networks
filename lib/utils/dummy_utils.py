...
def convert_to_graph(complex):
    ...
    # checks existing of at least nodes and at least one node
    assert 0 in complex.cochains
    assert complex.cochains[0].num_cells > 0

    cochain = complex.cochains[0] # nodes

    x = cochain.x # features
    y = complex.y # labels
    ...
    if cochain.upper_index is None:
        ...
        if 1 in complex.cochains and complex.cochains[1].x is not None and cochain.shared_coboundaries is not None:
            edge_attr = torch.index_select(complex.cochains[1].x, 0, cochain.shared_coboundaries)  # upper_index connections  (edge connections)
