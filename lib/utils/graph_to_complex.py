...
def build_tables(simplex_tree, size):
    ...
    id_maps = [{} for _ in range(complex_dim+1)]                                    # list with length n-dimension containing dictionary (simplex -> id). index -> dimension of the simplex
...
def extract_boundaries_and_coboundaries_from_simplex_tree(simplex_tree, id_maps, complex_dim: int):
    ...
    # summary of the structure returned by the above function
    # boundaries_tables : list of dictionaries. list of the ids of the boundaries. 
    # boundaries -> list of n-dimensions of dictionaries. maps simplex -> boundary 
    # list of n-dimension of dictionaires. maps simplex -> co_boundary
...
def build_adj(boundaries: List[Dict], coboundaries: List[Dict], id_maps: List[Dict], complex_dim: int,
        ...
        for simplex, id in id_maps[dim].items():
            ...
            if dim > 0:
                for boundary1, boundary2 in itertools.combinations(boundaries[dim][simplex], 2):        # get two boundaries of the current simplex
                    ...
                    upper_indexes[dim - 1].extend([[id1, id2], [id2, id1]])                             # equivalent with append twice. connect the lower complexes that share the current simplex as co_face
            ...
            if include_down_adj and dim < complex_dim and simplex in coboundaries[dim]:                 # make sure the simplex has co_boundaries
    ...
    return all_shared_boundaries, all_shared_coboundaries, lower_indexes, upper_indexes # the list of indexes of all boundaries to all the simplexes of upper dimension, and vice-versa, and then upper indices
...
def construct_features(vx: Tensor, cell_tables, init_method: str) -> List:
    ...
    for dim in range(1, len(cell_tables)):                                                              # Go through every dim >= 1
        ...
        aux_0 = []                                                                                      # storing cell
        for c, cell in enumerate(cell_tables[dim]): # count and cell complex
            aux_1 += [c for _ in range(len(cell))]                                                      # assinging id to veted of the cell. exntes the index_vertex
...
def extract_labels(y, size):
    ...
    y_shape = list(y.size())
    if y_shape[0] == 1 or y_shape == [math.comb(size, 2), 3]:# if there is only one label, then it is for the whoe graph TODO: change this. it is just for QT
        ...
        assert y_shape[0] == size # labels for all nodes
        v_y = y
    return v_y, complex_y # return 
...
def generate_cochain(dim, x, all_upper_index, all_lower_index,
    ...
    boundary_index = None
    if len(boundaries_tables[dim]) > 0: # if there are boundaries, of the current dimension
...
def compute_clique_complex_with_gudhi(x: Tensor, edge_index: Adj, size: int,
    ...
    print("before construction", y.shape, size)
...
def get_rings(edge_index, max_k=7): # get rings from a graph: edges of a graph and the maximum number of nodes in a ring
    ...
    for k in range(3, max_k+1):
        ...
        sub_isos = top.subgraph_isomorphism(pattern_gt, graph_gt, induced=True, subgraph=True,
                                           generator=True)              # induced rings only
...
def build_tables_with_rings(edge_index, simplex_tree, size, max_k):
    ...
    cell_tables, id_maps = build_tables(simplex_tree, size) # only for nodes and edges. 
    ...
    if len(rings) > 0:
        ...
        for cell in rings:
            next_id = len(cell_tables[2]) # number of edges
            id_maps[2][cell] = next_id # assing a new index to the ring
            cell_tables[2].append(list(cell)) # add the rings as edges
...
def get_ring_boundaries(ring):
    ...
    for n in range(len(ring)):  # nodes in the ring
        a = n
        if n + 1 == len(ring): # final node
            b = 0
        else:
            b = n + 1
...
def compute_path_complex(x: Union[Tensor, np.ndarray], edge_index: Union[Tensor, np.ndarray],
    ...
    cochains = []
    for i in range(complex_dim + 1):
        node_y = v_y if i == 0 else None
        cochain = generate_cochain(i, xs[i], upper_idx, lower_idx, shared_boundaries, shared_coboundaries,
                               path_tables, boundaries_tables, complex_dim=complex_dim, y=node_y)
        cochains.append(cochain)

    # print(complex_y.shape, 'shape of y passed to complex')
    return Complex(*cochains, y=complex_y, dimension=complex_dim)
