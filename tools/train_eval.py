...
from lib.helpers.eval_helpers import extract_results_molecular_datasets, extract_results_tu_datasets, extract_results_sr_datasets, extract_results_cosc_graphs
...
def main(args):
    ...
    if not args.sweep:
        result_folder = os.path.join(args.result_folder, args.dataset, args.model, 'runs', f'{time_stamp}-{args.exp_name}')
    else:
    ...
    elif (args.dataset == "cosc-graphs"):
        extract_results_cosc_graphs(args, results, result_folder)
...
def train_with_different_seeds(args, result_folder, device):
    ...
    for seed in range(args.start_seed, args.stop_seed + 1):
        ...
        if not args.model.endswith('cin'): # this is for normal graph models
            train_loader, valid_loader, test_loader, _, _, _, num_classes, num_features = get_dataloaders(args, fold = None)
            model = get_graph_model(args, num_features, num_classes, device)
        else: # for complex models
...
def train_with_different_folds(args, result_folder, device):
    results = []            # store statistics across different folds
    for fold in range(args.folds):
        # Data loading and Model initialization
        if not args.model.endswith('cin'):
            train_loader, valid_loader, test_loader, train_ids, val_ids, test_ids, num_classes, num_features = get_dataloaders(args, fold = fold)
            model = get_graph_model(args, num_features, num_classes, device)
            vals = ( train_loader, valid_loader, test_loader, train_ids, val_ids, test_ids)
        else:
            train_loader, valid_loader, test_loader, dataset = get_dataloaders(args, fold = fold)
            vals = (train_loader, valid_loader, test_loader, None, None, None)
            model = get_complex_model(args, dataset, device)
        ...
        # instantiate learning rate decay
        scheduler = get_lr_scheduler(args, optimizer)
        trainer = Trainer(model, args, *vals, optimizer, scheduler, result_folder, None, fold, device)
