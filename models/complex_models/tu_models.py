...
class SparseCIN(torch.nn.Module):
    ...
    def __init__(self, 
                 num_input_features, 
                 num_node_labels, # node labels
                 num_classes, # graph lables
                 ...
                 readout='sum',
                 train_eps=False, 
                 final_hidden_multiplier: int = 2, # multiplier of the final hidden layer. If we have an n-dimensional tensor as the final output, thenadd 
                 final_readout='sum', 
        ...
        # Find Readout Dims
        self.readout_dims = tuple([0, 1]) if dataset_name == "cosc-graphs" else tuple([dim for dim in readout_dims if dim <= max_dim])
        ...
        ## Add SparseCINConv, returns cochaimmessagepassingparams
        ...
        for _ in range(max_dim + 1):
            if self.jump_mode == 'cat': # if we are concatenating
        ...
        if dataset_name.startswith("cosc"):
            self.lin2s = torch.nn.Sequential(
                Linear(2 * hidden * final_hidden_multiplier, hidden),
                torch.nn.ReLU(),
                Linear(hidden, 3)   # 3 classes
            )
        else:
            if num_fc_layers == 2:
                self.lin2s = torch.nn.Sequential(
    ...
    def forward(self, data: ComplexBatch, include_partial=False):
        ...
        for c, conv in enumerate(self.convs):
            params = data.get_all_cochain_params(max_dim=self.max_dim, include_down_features=False) # a list of cochainMessagePassing params
            xs = conv(*params, start_to_process=0)
            data.set_xs(xs)

            if include_partial: # just storing partial node features during execution
        ...
        if not self.dataset_name.startswith("cosc"):
        ...
        if not self.dataset_name.startswith("cosc"):
            ...
            x = torch.as_tensor(new_xs[0])
        
        if self.apply_dropout_before == 'final_readout':
            x = F.dropout(x, p=self.dropout_rate, training=self.training)
        if self.dataset_name.startswith("cosc-graphs"):
        ...
        if self.dataset_name.startswith("cosc"):
            outputs = []
            ...
            for b in range(batch.max().item() + 1):
                ...
                xb = x[mask]
                n_nodes = xb.shape[0]
                a, b = torch.triu_indices(n_nodes, n_nodes, offset=1)
                pair_feat = torch.cat([xb[a], xb[b]], dim=1)
                logits = self.lin2s(pair_feat)
